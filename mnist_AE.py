...
for iIter in range(0,nIters):
	...
	optimizer.run(feed_dict={data:trainBatch})
	
	if iIter % logInterval == 0:
		[lossPerIm,summary] = sess.run([loss,summary_op],feed_dict={data:trainBatch})		
		summary_writer.add_summary(summary,iIter)
		avgLoss = lossPerIm / batchSize
		print("Iteration " + str(iIter) + ", train loss = " + str(avgLoss))

		[lossPerIm,summary] = sess.run([loss,summary_op],feed_dict={data:testBatch})		
